...
Gateway Caching (Semantic and Exact Match) and Provider Caching (cache read/write tokens) are different features. Gateway Caching is a policy configured at the gateway level. Provider Caching refers to the provider-side prompt caching reflected in token usage (e.g. `cache_read_input_tokens`). You MUST clarify which one the user is asking about.

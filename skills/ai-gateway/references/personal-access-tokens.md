...
> **Important**: Each PAT must be assigned to a **team**. The team **cannot** be `everyone` — you must use a specific named team. If the user is not part of any team, ask them to contact their admin to be added to one.
...
> **WARNING**: Do NOT use `apply_manifest` or `get_manifest_json_schema` for PATs. PATs have no manifest. You MUST use the `create_personal_access_token` tool directly.
...
Use `search_docs` to search for additional information about personal access tokens.
Search terms: "personal access token", "PAT creation", "API authentication", "token management"
